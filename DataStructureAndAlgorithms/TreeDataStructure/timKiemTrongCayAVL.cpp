...
int countNumber(Node* t, int x)
{
    if(t == NULL) return 0;
    if(t->data == x) return 1 + countNumber(t->left, x) + countNumber(t->right, x);
    else if(t->data < x) return countNumber(t->right, x);
    else return countNumber(t->left, x);
}
int main(){
    int n, x; cin >> n;
    Node* t = NULL;
